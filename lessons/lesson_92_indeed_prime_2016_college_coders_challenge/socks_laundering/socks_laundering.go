...
func Solution(K int, C []int, D []int) int {
    ...
    // If there are paired clean socks,
    // pair them first.
    for _, color := range C {
        if _, ok := c[color]; !ok {
            c[color] = true
    ...
    // Try to pair dirty socks with clean socks first,
    // which use 1 clean slot only.
    for color := range c {
        if num, ok := d[color]; ok && num > 0 && remain > 0 {
            d[color]--
            count++
            remain--
        }
    ...
    // No more clean socks can be paired, pairs dirty socks alone,
    // which use two clean slots.
    for _, num := range d {
        for remain >= 2 && num >= 2 {
            num -= 2
            count++
